...
(define (listen
  ...
  (define hb-socket      (zmq-socket context 'rep))
  (define shell-socket   (zmq-socket context 'router))
  (define control-socket (zmq-socket context 'router))
  (define iopub-socket   (zmq-socket context 'pub))
  (define stdin-socket   (zmq-socket context 'router))

  (define sockets (list hb-socket shell-socket control-socket stdin-socket))
  (define pollitems (make-zmq-pollitems 4 sockets))

  (zmq-bind iopub-socket iopub-endpoint)
  (for-each zmq-bind sockets endpoints)
